...
function FocusManager:inputEvent(action, value, eventUsed)
    ...
    local pressedUp = action == InputAction.MENU_AXIS_UP_DOWN and value > g_analogStickVTolerance
    ...
    local pressedRight = action == InputAction.MENU_AXIS_LEFT_RIGHT and value > g_analogStickHTolerance
    ...
    if not eventUsed and element ~= nil and not element.needExternalClick then
        ...
        if pressedAccept and not self:isFocusInputLocked(action) then
            ...
            if element.focusActive and element:getIsVisible() then
...
-- @param True if locked, false otherwise
...
function FocusManager.getShortestBoundingBoxVector(elementBox, otherBox, otherCenter)
    local ePointX, ePointY = FocusManager.getClosestPointOnBoundingBox(
        otherCenter[1], otherCenter[2],
        elementBox[1], elementBox[2], elementBox[3], elementBox[4])

    local oPointX, oPointY = FocusManager.getClosestPointOnBoundingBox(
        ePointX, ePointY, -- use the previously calculated bounding box point here to get the closest boundary distance
        otherBox[1], otherBox[2], otherBox[3], otherBox[4])
    ...
    if isFocusMoving then
        if self.lastInput[direction] then
            if self.lockUntil[direction] <= g_time then
                self.lastInput[direction] = nil -- release lock
                self.lockJustReleased = true
            end
            -- movement not allowed yet
            return
        end
...
function FocusManager:unsetHighlight(element)
    if self.currentFocusData.highlightElement and self.currentFocusData.highlightElement == element then
        ...
        if prevState == GuiOverlay.STATE_HIGHLIGHTED then
            ...
            self.currentFocusData.highlightElement:restoreOverlayState() -- also try restoring
        end
...
function FocusManager:setFocus(element, direction, ...)
    ...
    if self.currentFocusData.focusElement and
            ...
            self.currentFocusData.focusElement.focusActive then
...
-- @param isFocus If true, the element's state will be set to focused. Otherwise, it's state will be either restored or set to normal.
-- @param handlePreviousState [optional] If true or undefined, makes the element store its previous overlay state before modification or restore it when isFocused is false.
function FocusManager:setElementFocusOverlayState(element, isFocused, handlePreviousState)
    ...
    if isFocused then
        ...
        if element:getOverlayState() == GuiOverlay.STATE_FOCUSED then -- could still be focused state after restore, just set to normal

...
function InGameIcon.new()
    ...
    instance.textPosX = instance.posX + instance.width * (3/5)
...
function InGameIcon:mouseEvent(posX, posY, isDown, isUp, button)
    if self.visible and isDown and button == 1 then
...
function InGameIcon:update(dt)
    if self.visible then
        ...
        -- fade in
        self.alpha = math.min(1.0, self.time / self.fadeTime)

        -- fade out
        if self.time > self.fadeTime + self.visibleTime then
            self.alpha = math.max(0.0, (self.fadeTime - (self.time - self.fadeTime - self.visibleTime)) / self.fadeTime)
        end
...
function InGameIcon:setPosition(x, y)
    ...
    self.textPosX = self.posX + self.width * (3/5)
...
function InGameIcon:draw()
    if self.visible then
        ...
        setTextColor(1.0, 1.0, 1.0, self.alpha)

...
function TextInputElement:loadFromXML(xmlFile, key)
    ...
    self.maxInputTextWidth = unpack(GuiUtils.getNormalizedValues(getXMLString(xmlFile, key.."#maxInputTextWidth"), {self.outputSize[1]}, {self.maxInputTextWidth}))
...
function TextInputElement:loadProfile(profile, applyProfile)
    ...
    self.maxInputTextWidth = unpack(GuiUtils.getNormalizedValues(profile:getValue("maxInputTextWidth"), {self.outputSize[1]}, {self.maxInputTextWidth}))
...
function TextInputElement:mouseEvent(posX, posY, isDown, isUp, button, eventUsed)
    if self:getIsActive() then
        ...
        elseif self.enterWhenClickOutside then
...
function TextInputElement:deleteText(deleteRightCharacterFromCursor)
    ...
    if textLength > 0 then
        ...
        if canDelete then
            --[[
            print(string.format(
            "self.cursorPosition %d, deleteOffset %d, textLength %d, prestring : %s, poststring : %s",
            self.cursorPosition, deleteOffset, textLength,
            tostring(utf8Substr(self.text, 0, self.cursorPosition + deleteOffset - 1)),
            tostring(utf8Substr(self.text, self.cursorPosition + deleteOffset, -1))))
            --]]
            self.text =
...
function TextInputElement:inputEvent(action, value, eventUsed)
    ...
    if self.blockTime <= 0 then
        if not self.imeActive and self:getIsActive() and self:getOverlayState() == GuiOverlay.STATE_PRESSED then
...
function TextInputElement:updateVisibleTextElements()
    ...
    if self:getIsActive() and self:getOverlayState() == GuiOverlay.STATE_PRESSED then
        ...
        if availableTextWidth and self.cursorPosition > self.firstVisibleCharacterPosition + visibleTextLength then
            ...
            visibleText = TextInputElement.limitTextToAvailableWidth(
                    textTrimmedAtCursor, self.textSize, availableTextWidth, true)

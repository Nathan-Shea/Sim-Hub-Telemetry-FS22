...
function SliderElement:loadFromXML(xmlFile, key)
    ...
    self.sliderOffset = unpack(GuiUtils.getNormalizedValues(getXMLString(xmlFile, key.."#sliderOffset"), {self.outputSize[2]}, {self.sliderOffset}))
...
function SliderElement:loadProfile(profile, applyProfile)
    ...
    self.sliderOffset = unpack(GuiUtils.getNormalizedValues(profile:getValue("sliderOffset"), {self.outputSize[2]}, {self.sliderOffset}))
...
function SliderElement:onBindUpdate(element)
    if element:isa(ListElement) then
        ...
        self.needsSlider = numItems > numVisibleItems
    elseif element:isa(ScrollingLayoutElement) then
        ...
        -- self:setValue(element.firstVisibleY element.contentSize)
    elseif element:isa(SmoothListElement) then
        ...
        self.needsSlider = element.contentSize > viewSize

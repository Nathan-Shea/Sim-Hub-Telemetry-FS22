...
---Set the force highlight state of this element.
If force highlight is enabled, this element will not be highlighted automatically but must receive the highlight
state manually by calls to FocusManager:setHighlight().
...
function MultiTextOptionElement:addElement(element)
    ...
    if self.namedComponents then
        ...
        -- TODO: this is super easy to get wrong when writing configurations from scratch. Make it smarter, e.g. using names to resolve collaborators.
        ...
        elseif numElements == 6 or numElements == 7 then
            if self.textElement ~= nil and self.textElement ~= nil then
                if element:isa(BitmapElement) then
                    table.insert(self.gradientElements, element)
                end
            end

...
function FlowLayoutElement:invalidateLayout(ignoreVisibility)
    ...
    for currentIndex, element in ipairs(self.elements) do
        if self:getIsElementIncluded(element, ignoreVisibility) then
            ...
            -- Assume it goes on current line
            element.line = currentLine

            -- Check wrapping but only on left alignment, because the math does not support the others
            if ((x + element.absSize[1]) - self.absSize[1]) > 0.0001 and self.alignmentX == FlowLayoutElement.ALIGN_LEFT then
                x = 0
                currentLine = currentLine + 1

                -- Find where we can cut
                local cutIndex = currentIndex -- left of N
                for i = currentIndex, 1, -1 do
                    local testElement = self.elements[i]
                    if testElement.line ~= nil or i == currentIndex then
                        if not testElement.disallowFlowCut then
                            cutIndex = i
                            break
                        end
                    end
                ...
                for i = cutIndex, math.max(currentIndex - 1, 1) do -- skip currentIndex, we do that below
            ...
            -- used to quickly check inclusion in next steps
            element.line = nil
        end
    end
    ...
    for _, element in pairs(self.elements) do
        if element.line ~= nil then
            -- Inverted Y line as 0 is at bottom

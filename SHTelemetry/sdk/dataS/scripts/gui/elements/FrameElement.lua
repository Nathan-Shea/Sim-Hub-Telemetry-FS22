...
function FrameElement:registerControls(controlIDs)
    for _, id in pairs(controlIDs) do
        if self.controlIDs[id] then
            ...
            self.controlIDs[id] = false -- hash it for ease of access
        end
    end
end
...
function FrameElement:disableInputForDuration(duration)
    self.inputDisableTime = MathUtil.clamp(duration, 0, 10000) -- clamp to plausible range (up to 10s)
end
...
-- @param func callback Function reference, signature: function(sourceFrameElement, targetScreenClass, returnScreenClass)
...
-- @param func callback Function reference, signature: function(isContextActive)
...
-- @param func callback Function reference, signature: function(sampleName)
...
-- @param bool isContextActive If true, will activate a custom menu input context. Otherwise, will clear a previously
activated context.

...
function AIParameterUnloadingStation:loadFromXMLFile(xmlFile, key)
    ...
    if unloadingStationSavegameId ~= nil and stationIndex ~= nil then
        if not self:setUnloadingStationFromSavegameId(unloadingStationSavegameId, stationIndex) then
            g_messageCenter:subscribeOneshot(MessageType.LOADED_ALL_SAVEGAME_PLACEABLES, self.onPlaceableLoaded, self, {unloadingStationSavegameId, stationIndex})
        end
    end
end

...
function AIParameterLoadingStation:loadFromXMLFile(xmlFile, key)
    ...
    if loadingStationSavegameId ~= nil and stationIndex ~= nil then
        if not self:setLoadingStationFromSavegameId(loadingStationSavegameId, stationIndex) then
            g_messageCenter:subscribeOneshot(MessageType.LOADED_ALL_SAVEGAME_PLACEABLES, self.onPlaceableLoaded, self, {loadingStationSavegameId, stationIndex})
        end
    end
end

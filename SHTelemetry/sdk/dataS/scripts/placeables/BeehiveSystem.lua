...
-- @param farmId integer optional filter for farm id
...
function BeehiveSystem:updateState()
    ...
    elseif environment.currentSeason == Environment.SEASON.WINTER then
        self.isFxActive = false
        self.isProductionActive = false  -- honey production is continuous unless it's winter
...
function BeehiveSystem:getBeehiveInfluenceFactorAt(wx, wz)
    ...
    --#debug     local wy = getTerrainHeightAtWorldPos(g_currentMission.terrainRootNode, wx, 0, wz)

        ...
        if spec.sellingStationPlaceable == nil then
            if spec.drivingRangeSellingStationId ~= nil then
                for _, placeable in pairs(g_currentMission.placeableSystem.placeables) do
                    if placeable.mapBoundId == spec.drivingRangeSellingStationId then
        ...
        if spec.showDialogDelay > 0 then
            ...
            if spec.showDialogDelay <= 0 then
                ...
                if spec.showDialog == 2 then
                    g_gui:showYesNoDialog({text=textDriveInfo .. "\n\n" .. textSellQuestion, callback=self.onSellGoodsQuestion, target=self})
                else
                    g_gui:showInfoDialog({text=textDriveInfo})
                end
...
--#debug                     Logging.devWarning("Vehicle '%s' (savegameId %d) is not a railroad vehicle", vehicle:getName(), id)
--#debug                     printCallstack()
        ...
        for k, filename in ipairs(spec.vehiclesToLoad) do
            ...
            VehicleLoadingUtil.loadVehicle(filename, {x=0, z=0, yOffset=0}, true, 0, Vehicle.PROPERTY_STATE_NONE, AccessHandler.EVERYONE, nil, nil, spec.railroadVehicleLoaded, self, arguments)
        end

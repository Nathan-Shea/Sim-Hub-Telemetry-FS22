...
function PlaceableSilo:onFinalizePlacement()
    ...
    if not spec.storagePerFarm then
        -- For the very first silo on easy, fill it. The startSiloAmounts are only set on game creation.
        -- In case of no silo on the map this code is used for the first silo placed in new savegame.
        local num = 0
        ...
        if num == 1 and g_currentMission.missionInfo.difficulty == 1 and g_currentMission.missionInfo.startSiloAmounts ~= nil and not g_currentMission.missionInfo:getIsLoadedFromSavegame() and not g_currentMission.missionInfo.hasLoadedFirstFilledSilo then
            -- This is to circumvent a cheat: load new easy game, sell silo, place silo (it is filled now), sell silo, etc.
            -- No  need to save it as this code does not run on savegames
...
function PlaceableSilo:updateInfo(superFunc, infoTable)
    ...
    if numEntries > 0 then
        ...
        table.insert(infoTable, {title=g_i18n:getText("infohud_siloEmpty"), text=""})
    end
end

...
function PlaceableLights:updateLightState(groupIndex, isActive)
    ...
    for _, realLight in ipairs(activeLightSetup) do
        if realLight.groupIndex == groupIndex then
            setVisibility(realLight.node, not group.hasManualLights or isActive) -- lights are always active for groups which cannot be toggled manually (visiblity condition)
        end
    end

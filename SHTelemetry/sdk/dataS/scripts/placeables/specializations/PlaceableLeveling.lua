...
function PlaceableLeveling:onDelete()
    ...
    if spec.writtenBlockedAreas then
        -- unblock areas in the blocked area map
...
function PlaceableLeveling:onPreFinalizePlacement()
    local spec = self.spec_leveling
    -- block areas in the blocked area map to prevent placeable overlaps
    local deformationObjects = self:getDeformationObjects(g_currentMission.terrainRootNode, true, false)
    for _, deformationObject in ipairs(deformationObjects) do
...
-- @param int terrainRootNode Map terrain root node ID
-- @param bool forBlockingOnly Create the object only to write into the blocking map
-- @param bool isBlocking If forBlockingOnly is true, this also tells the method if we intend to block areas (true) or unblock them (false)

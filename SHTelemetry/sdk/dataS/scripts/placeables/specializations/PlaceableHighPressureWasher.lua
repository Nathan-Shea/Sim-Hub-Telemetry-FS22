...
function PlaceableHighPressureWasher:onLoad(savegame)
    ...
    if self.isClient then
        spec.samples = {}
        spec.samples.compressor = g_soundManager:loadSampleFromXML(xmlFile, "placeable.highPressureWasher.sounds", "compressor", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
        spec.samples.switch = g_soundManager:loadSampleFromXML(xmlFile, "placeable.highPressureWasher.sounds", "switch", self.baseDirectory, self.components, 1, AudioGroup.VEHICLE, self.i3dMappings, self)
    end
    ...
    if spec.isTurnedOn ~= isTurnedOn then
        if noEventSend == nil or noEventSend == false then
            if g_server ~= nil then
                g_server:broadcastEvent(PlaceableHighPressureWasherStateEvent.new(self, isTurnedOn, player), nil, nil, self)
            else

        ...
        if self.type ~= nil then
            ...
            xmlFile:iterate(key .. ".variations.variation", function(_, variationKey)
                ...
                xmlFile:iterate(variationKey .. ".color", function(_, colorKey)
                    local color = {}
                    color.shaderParameter = xmlFile:getValue(colorKey .. "#shaderParameter")
                    color.color = xmlFile:getValue(colorKey .. "#color", nil, true)
...
function LicensePlate:setVariation(variationIndex)
    ...
    if variation ~= nil then
        ...
        for i=1, #variation.values do
            ...
            if not value.isStatic then
                value.singleCharacter = self.font:createSingleCharacter(value.node, self.fontSize, {0, 0, 0, 1}, nil, 0, self.fontScaleX, self.fontScaleY, 0)
            end
        end

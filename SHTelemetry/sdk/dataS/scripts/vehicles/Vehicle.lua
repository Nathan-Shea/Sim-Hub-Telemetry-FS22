...
function Vehicle.init()
    g_configurationManager:addConfigurationType("baseColor",   g_i18n:getText("configuration_baseColor"),   nil, nil, ConfigurationUtil.getConfigColorSingleItemLoad, ConfigurationUtil.getConfigColorPostLoad, ConfigurationUtil.SELECTOR_COLOR)
    g_configurationManager:addConfigurationType("design",      g_i18n:getText("configuration_design"),      nil, nil, ConfigurationUtil.getConfigColorSingleItemLoad, ConfigurationUtil.getConfigColorPostLoad, ConfigurationUtil.SELECTOR_MULTIOPTION)
    g_configurationManager:addConfigurationType("designColor", g_i18n:getText("configuration_designColor"), nil, nil, ConfigurationUtil.getConfigColorSingleItemLoad, ConfigurationUtil.getConfigColorPostLoad, ConfigurationUtil.SELECTOR_COLOR)
    g_configurationManager:addConfigurationType("vehicleType", g_i18n:getText("configuration_design"),      nil, nil, ConfigurationUtil.getStoreAdditionalConfigData,  nil,                                      ConfigurationUtil.SELECTOR_MULTIOPTION)
...
function Vehicle:load(vehicleData, asyncCallbackFunction, asyncCallbackObject, asyncCallbackArguments)
    ...
    local data = {}
    data.position = {posX=vehicleData.posX, posY=vehicleData.posY, posZ=vehicleData.posZ, yOffset=vehicleData.yOffset, isAbsolute=vehicleData.isAbsolute}
    data.rotation = {rotX=vehicleData.rotX, rotY=vehicleData.rotY, rotZ=vehicleData.rotZ}
    data.isVehicleSaved = vehicleData.isVehicleSaved
    data.propertyState = vehicleData.propertyState
    data.ownerFarmId = vehicleData.ownerFarmId
    data.price = vehicleData.price
    data.savegame = vehicleData.savegame
    data.asyncCallbackFunction = asyncCallbackFunction
    data.asyncCallbackObject = asyncCallbackObject
    data.asyncCallbackArguments  = asyncCallbackArguments
    data.componentPositions  = vehicleData.componentPositions
    ...
    if item ~= nil and item.configurations ~= nil then
        ...
        if item.configurationSets ~= nil and #item.configurationSets > 0 then
            if not StoreItemUtil.getConfigurationsMatchConfigSets(self.configurations, item.configurationSets) then
...
function Vehicle:loadFinished(i3dNode, failedReason, arguments, i3dLoadingId)
    ...
    while true do
        ...
        local component = {node = getChildAt(i3dNode, 0)}
    ...
    self.thresholdTailwaterDepthWarning = self.xmlFile:getValue("vehicle.base.tailwaterDepth#warning", 1.0)
    ...
    if preLoadDesignConfigurations then
    ...
    -- apply material from all configurations
    ...
    -- apply design
    if not preLoadDesignConfigurations then
        if self.configurations["design"] ~= nil then
            ObjectChangeUtil.updateObjectChanges(self.xmlFile, "vehicle.designConfigurations.designConfiguration", self.configurations["design"], self.components, self)
        end
    end
            ...
            if isAbsolute then
                ...
                while true do
                    ...
                    if componentPosition[componentIndex] ~= nil then
                        ...
                        componentPosition[componentIndex] = {x=x, y=y, z=z, xRot=xRot, yRot=yRot, zRot=zRot}
                    end
                    i = i + 1
                end
    ...
    self.subLoadingTasksFinishedAsyncData = {asyncCallbackFunction=asyncCallbackFunction,
...
function Vehicle:loadSubSharedI3DFile(filename, callOnCreate, addToPhysics, asyncCallbackFunction, asyncCallbackObject, asyncCallbackArguments)
    if asyncCallbackFunction ~= nil and not self.syncVehicleLoadingFinished then
        ...
        asyncCallbackFunction = function(target, i3dNode, ...)
...
function Vehicle:readStream(streamId, connection, objectId)
    ...
    if self.configFileName == nil then
        local vehicleData = {}
        vehicleData.filename = configFile
        vehicleData.isAbsolute = false
        vehicleData.typeName = typeName
        vehicleData.posX = 0
        vehicleData.posY = nil
        vehicleData.posZ = 0
        vehicleData.yOffset = 0
        vehicleData.rotX = 0
        vehicleData.rotY = 0
        vehicleData.rotZ = 0
        vehicleData.isVehicleSaved = true
        vehicleData.price = 0
        vehicleData.propertyState = self.propertyState
        -- assign parent class Object's ownerFarmId field here to ensure ownership synchronization in MP:
        vehicleData.ownerFarmId = self.ownerFarmId
        vehicleData.isLeased = 0
        vehicleData.configurations = configurations
        vehicleData.boughtConfigurations = boughtConfigurations

        local vehicle = self
        local asyncCallbackFunction = function(_, v, loadingState, args)
...
function Vehicle:draw(subDraw)
    if self:getIsSynchronized() then
        ...
        -- DebugUtil.drawDebugNode(self.rootNode, "farm: " .. tostring(self:getOwnerFarmId()) .. ", controller: " .. tostring(self:getActiveFarm()))
...
function Vehicle:setLoadingStep(loadingStep)
    ...
    or loadingStep == Vehicle.LOAD_STEP_PRE_LOAD
    or loadingStep == Vehicle.LOAD_STEP_AWAIT_I3D
    or loadingStep == Vehicle.LOAD_STEP_LOAD
    or loadingStep == Vehicle.LOAD_STEP_POST_LOAD
    or loadingStep == Vehicle.LOAD_STEP_AWAIT_SUB_I3D
    or loadingStep == Vehicle.LOAD_STEP_FINISHED
    or loadingStep == Vehicle.LOAD_STEP_SYNCHRONIZED then
...
function Vehicle:onWaterRaycastCallback(waterY, args)
    ...
    self.waterY = waterY
    self.isInWater = waterY > y
    self.isInShallowWater = false
    self.isInMediumWater = false
    if self.isInWater then
        local x, z = self.waterCheckPosition[1], self.waterCheckPosition[3]
...
function Vehicle:getMaxComponentMassReached()
    return (self.serverMass + 0.00001) >= (self.maxComponentMass - self.precalculatedMass)
end
...
-- @param table fillLevelInformations fill level informations
...
function Vehicle:setOperatingTime(operatingTime, isLoading)
    if not isLoading and self.propertyState == Vehicle.PROPERTY_STATE_LEASED and g_currentMission ~= nil and g_currentMission.economyManager ~= nil and math.floor(operatingTime/(1000*60*60)) > math.floor(self.operatingTime/(1000*60*60)) then

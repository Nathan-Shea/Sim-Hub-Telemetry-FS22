...
function TensionBelts.initSpecialization()
    g_configurationManager:addConfigurationType("tensionBelts", g_i18n:getText("configuration_tensionBelts"), "tensionBelts", nil, nil, nil, ConfigurationUtil.SELECTOR_MULTIOPTION)
...
function TensionBelts:onUpdate(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
    ...
    if self.isClient then
        if spec.fastedAllBeltsIndex > 0 then
...
function TensionBelts:lockTensionBeltObject(objectId, objectsToJointTable, isDynamic, jointNode, object)
    if objectsToJointTable[objectId] == nil then
        ...
        if useDynamicMount then
            ...
            objectsToJointTable[objectId] = {jointIndex=jointIndex, jointTransform=jointTransform, object=object}
...
-- @param table object objects to fasten
-- @param boolean playSound play sound (nil = true)
...
function TensionBelts:objectOverlapCallback(transformId)
    if transformId ~= 0 and getHasClassId(transformId, ClassIds.SHAPE) then
        ...
        if object ~= nil and object ~= self and (object.rootVehicle ~= self.rootVehicle) then
            if object.getSupportsTensionBelts ~= nil and object:getSupportsTensionBelts() then
                if object.getMeshNodes ~= nil and object.dynamicMountObject == nil then
                    local nodeId = object:getTensionBeltNodeId()
                    if spec.objectsInTensionBeltRange[nodeId] == nil then
                        local nodes = object:getMeshNodes()
                        if nodes ~= nil then
                            spec.objectsInTensionBeltRange[nodeId] = {physics=nodeId, visuals=nodes, object=object}
...
function TensionBelts:getIsFoldAllowed(superFunc, direction, onAiTurnOn)
    local spec = self.spec_tensionBelts
    if not spec.allowFoldingWhileFasten then
        if not (direction < 0 or not spec.areBeltsFasten) then

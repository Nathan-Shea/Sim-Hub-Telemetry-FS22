...
function Cutter:onLoad(savegame)
    ...
    if self.isClient then
        ...
        while true do
            ...
            local extraObject = {node=node, anim=anim}
    ...
    if self:loadCutterTiltFromXML(self.xmlFile, Cutter.CUTTER_TILT_XML_KEY, spec.automaticTilt) then
        ...
        spec.automaticTilt.lastHit = {0, 0, 0}
    ...
    spec.workAreaParameters.fruitTypesToUse = {}
        ...
        if self.isActive then
            for i=1, #spec.automaticTilt.nodes do
                ...
                if not isActive and doReset then
                    currentDelta = -curZ * 0.1 -- return to idle is not active
                end
...
function Cutter:onUpdateTick(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
    ...
    if isActive then
        if automaticTilt ~= nil and automaticTilt.raycastNode1 ~= nil and automaticTilt.raycastNode2 ~= nil then
            ...
            local hit1X, hit1Y, hit1Z = automaticTilt.lastHit[1], automaticTilt.lastHit[2], automaticTilt.lastHit[3]
            ...
            automaticTilt.raycastHit = false
            raycastAll(rx, ry, rz, rDirX, rDirY, rDirZ, "tiltRaycastDetectionCallback", 2, self, Cutter.AUTO_TILT_COLLISION_MASK)
            local hit2X, hit2Y, hit2Z = automaticTilt.lastHit[1], automaticTilt.lastHit[2], automaticTilt.lastHit[3]
            ...
            --#debug     DebugUtil.drawDebugGizmoAtWorldPos(hit1X, hit1Y, hit1Z, 0, 0, 1, 0, 1, 0, "r1", false)
            --#debug     DebugUtil.drawDebugGizmoAtWorldPos(hit2X, hit2Y, hit2Z, 0, 0, 1, 0, 1, 0, "r2", false)
...
-- @return bool success successfully loaded
...
function Cutter:isAttachAllowed(superFunc, farmId, attacherVehicle)
    ...
    if attacherVehicle.spec_combine ~= nil then
        if not attacherVehicle:getIsCutterCompatible(spec.fillTypes) then
...
---
function Cutter.updateExtraObjects(self)
    local spec = self.spec_cutter
...
function Cutter:updateDebugValues(values)
    ...
    for fillType, value in pairs(spec.lastOutputFillTypes) do
        table.insert(values, {name=string.format("buffer (%s)", g_fillTypeManager:getFillTypeNameByIndex(fillType)), value=string.format("%.0f%%", value/sum*100)})
    end
end

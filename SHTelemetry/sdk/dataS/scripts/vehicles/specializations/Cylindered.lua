...
function Cylindered.initSpecialization()
    ...
    schema:register(XMLValueType.FLOAT, "vehicle.cylindered.movingTools.easyArmControl.zTranslationNodes#moveRatioMinDir", "Defines direction value when the translation parts start to move", 0.0)
    schema:register(XMLValueType.FLOAT, "vehicle.cylindered.movingTools.easyArmControl.zTranslationNodes#moveRatioMaxDir", "Defines direction value when the rotation parts stop to move", 1.0)
...
function Cylindered:onLoad(savegame)
    ...
    if self.loadDashboardsFromXML ~= nil then
        local dashboardData = {valueTypeToLoad = "movingTool",
                               valueObject = self,
...
function Cylindered:onWriteUpdateStream(streamId, connection, dirtyMask)
    ...
    if connection:getIsServer() then
        ...
        if streamWriteBool(streamId, bitAND(dirtyMask, spec.cylinderedDirtyFlag) ~= 0) then
            for _, tool in ipairs(spec.movingTools) do
                if tool.dirtyFlag ~= nil then
                    if streamWriteBool(streamId, bitAND(dirtyMask, tool.dirtyFlag) ~= 0 and self:getIsMovingToolActive(tool)) then
                        if tool.transSpeed ~= nil then
                            streamWriteFloat32(streamId, tool.curTrans[tool.translationAxis])
                        end
                        if tool.rotSpeed ~= nil then
...
function Cylindered:updateEasyControl(dt, updateDelayedNodes)
    ...
    if easyArmControl ~= nil then
        ...
        if self.isServer and easyArmControl.state and hasChanged then
            ...
            --#debug DebugUtil.drawDebugGizmoAtWorldPos(c1xw, c1yw, c1zw, 0, 0, 1, 0, 1, 0, "c1", false)
            --#debug local c2xw, c2yw, c2zw = localToWorld(easyArmControl.rootNode, circle2X, circle2Y, circle2Z)
            --#debug DebugUtil.drawDebugGizmoAtWorldPos(c2xw, c2yw, c2zw, 0, 0, 1, 0, 1, 0, "c2", false)

            --#debug DebugUtil.drawDebugGizmoAtWorldPos(c2xw, c2yw - 0.5, c2zw, 0, 0, 1, 0, 1, 0, string.format("move: %.2f %.2f", moveY / dt * 1000, moveZ / dt * 1000), false)
            ...
            if numZTranslationNodes > 0 then
                ...
                --#debug DebugUtil.drawDebugGizmoAtWorldPos(c2xw, c2yw - 1, c2zw, 0, 0, 1, 0, 1, 0, string.format("transDirZ %.2f\n rotTransRatio: %.2f\n transMove: %.3f\n zDifference: %.3f\n", transDirZ, rotTransRatio, transMove / dt * 1000, zDifference / dt * 1000), false)
                ...
                for i=1, numZTranslationNodes do
                    ...
                    --#debug  DebugUtil.drawDebugNode(zTranslationNode.node, "trans"..i, false)
                end
            ...
            if ix ~= nil and iy ~= nil then
                ...
                --#debug DebugUtil.drawDebugGizmoAtWorldPos(dx1, dy1, dz1, 0, 0, 1, 0, 1, 0, "i1", false)
                --#debug DebugUtil.drawDebugGizmoAtWorldPos(dx2, dy2, dz2, 0, 0, 1, 0, 1, 0, "i2", false)
    ...
    if #part.dependentAnimations > 0 then
        for _, dependentAnimation in ipairs(part.dependentAnimations) do
            ...
            if dependentAnimation.translationAxis ~= nil then
                local retValues = {getTranslation(dependentAnimation.node)}
            ...
            if dependentAnimation.rotationAxis ~= nil then
                local retValues = {getRotation(dependentAnimation.node)}
...
function Cylindered:loadCopyLocalDirectionParts(xmlFile, baseName, entry)
    ...
    while true do
        ...
        if node ~= nil then
            ...
            copyLocalDirectionPart.dirScale = xmlFile:getValue(refBaseName.."#dirScale", nil, true)
            copyLocalDirectionPart.upScale = xmlFile:getValue(refBaseName.."#upScale", nil, true)
...
-- @param bool forceUpdate force immediate update of moving tool and dependent parts
...
function Cylindered:isDetachAllowed(superFunc)
    ...
    if spec.detachLockNodes ~= nil then
        for entry, data in pairs(spec.detachLockNodes) do
            ...
            if data.detachingRotMaxLimit ~= nil and rot[entry.rotationAxis] > data.detachingRotMaxLimit then
                return false, nil
            end
            ...
            if data.detachingTransMaxLimit ~= nil and trans[entry.translationAxis] > data.detachingTransMaxLimit then
                return false, nil
            end
        end
    end
...
-- @param bool updateDelayedNodes update delayed nodes
    ...
    if part.referencePoint ~= nil then
        ...
        if part.doInversedLineAlignment then
            ...
            for i=1, #part.orientationLineNodes-1 do
                ...
                if targetLength >= minLength and targetLength <= maxLength then
    ...
    if part.referencePoint ~= nil then
        ...
        if numTranslatingParts > 0 then
            ...
            for i=1,numTranslatingParts do
                ...
                local allowUpdate = true
                if part.referenceDistanceThreshold > 0 then
                    if math.abs(translatingPart.lastZ-newZ) < part.referenceDistanceThreshold then
                        allowUpdate = false
                    end
                end
...
---
function Cylindered.movingToolDashboardAttributes(self, xmlFile, key, dashboard)

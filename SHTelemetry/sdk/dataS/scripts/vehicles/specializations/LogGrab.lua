...
function LogGrab:onLoad(savegame)
    ...
    if self.isServer then
        ...
        while true do
            ...
            local entry = {}
            entry.componentJoint =  self.xmlFile:getValue(baseKey.."#componentJoint")
            entry.dampingFactor =  self.xmlFile:getValue(baseKey.."#dampingFactor", 20)
            entry.axis =  self.xmlFile:getValue(baseKey.."#axis", 1)
            ...
            if componentJoint ~= nil then
                entry.startRotDifference = {localDirectionToLocal(self.components[componentJoint.componentIndices[2]].node, componentJoint.jointNode, unpack(entry.direction))}
            end
        ...
        for shape,_ in pairs(spec.pendingDynamicMountShapes) do
            if spec.dynamicMountedShapes[shape] == nil then
                ...
                if jointIndex ~= nil then
                    spec.dynamicMountedShapes[shape] = {jointIndex=jointIndex, jointTransform=jointTransform}
                    spec.pendingDynamicMountShapes[shape] = nil
                end
            end
        end
...
function LogGrab:updateLogGrabState(grab, dt)
    ...
    if componentJoint ~= nil then
        ...
        if math.abs(currentOffset) > grab.rotationOffsetThreshold then
            if grab.rotationOffsetTimer > grab.rotationOffsetTime then
                return true
            else
        ...
        elseif grab.rotationOffsetTimer > 0 then
            ...
            local rotSum = x + y + z
            -- only unmount if the rotation of the componentJoint has changed -> if user opens the grab
...
function LogGrab:updateDebugValues(values)
    ...
    if self.isServer then
        for i, grab in ipairs(spec.grabs) do
            table.insert(values, {name=string.format("grab (%d):", i), value=string.format("current: %.2fdeg / threshold: %.2fdeg  (timer: %d)", math.deg(math.abs(grab.currentOffset)), math.deg(grab.rotationOffsetThreshold), grab.rotationOffsetTimer)})
        end

        for shapeId, _ in pairs(spec.dynamicMountedShapes) do
            if entityExists(shapeId) then
                table.insert(values, {name="mounted: ", value=tostring(getName(shapeId))})
            end
        end

        for shapeId, _ in pairs(spec.pendingDynamicMountShapes) do
            if entityExists(shapeId) then
                table.insert(values, {name="pending: ", value=tostring(getName(shapeId))})
            end
        end
    end
end

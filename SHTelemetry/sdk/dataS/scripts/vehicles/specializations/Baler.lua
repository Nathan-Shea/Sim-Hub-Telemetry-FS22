...
function Baler.initSpecialization()
    ...
    schema:register(XMLValueType.FLOAT, "vehicle.baler.additives#usage", "Usage per picked up liter", 0.0000275)
...
function Baler:onLoad(savegame)
    ...
    if self.isClient then
        spec.samples = {}
        spec.samples.work                   = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.baler.sounds", "work", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
        spec.samples.eject                  = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.baler.sounds", "eject", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
        spec.samples.unload                 = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.baler.sounds", "unload", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
        spec.samples.door                   = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.baler.sounds", "door", self.baseDirectory, self.components, 1, AudioGroup.VEHICLE, self.i3dMappings, self)
        spec.samples.knotCleaning           = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.baler.sounds", "knotCleaning", self.baseDirectory, self.components, 1, AudioGroup.VEHICLE, self.i3dMappings, self)
    ...
    spec.buffer = {}
    spec.buffer.fillUnitIndex = self.xmlFile:getValue("vehicle.baler.buffer#fillUnitIndex")
    spec.buffer.unloadInfoIndex = self.xmlFile:getValue("vehicle.baler.buffer#unloadInfoIndex", 1)
    spec.buffer.capacityPercentage = self.xmlFile:getValue("vehicle.baler.buffer#capacityPercentage")
    spec.buffer.overloadingDuration = self.xmlFile:getValue("vehicle.baler.buffer#overloadingDuration", 1)
    spec.buffer.overloadingDelay = self.xmlFile:getValue("vehicle.baler.buffer#overloadingDelay", 0)
    spec.buffer.overloadingTimer = 0
    spec.buffer.overloadingStartFillLevelPct = MathUtil.round(self.xmlFile:getValue("vehicle.baler.buffer#overloadingStartFillLevelPct", 1), 2)
    spec.buffer.fillMainUnitAfterOverload = self.xmlFile:getValue("vehicle.baler.buffer#fillMainUnitAfterOverload", false)
    ...
    spec.variableSpeedLimit.enabled = self.xmlFile:hasProperty("vehicle.baler.variableSpeedLimit")
    spec.variableSpeedLimit.pickupPerSecond = 0
    spec.variableSpeedLimit.pickupPerSecondTimer = 0
    spec.variableSpeedLimit.targetLiterPerSecond = self.xmlFile:getValue("vehicle.baler.variableSpeedLimit#targetLiterPerSecond", 200)
    spec.variableSpeedLimit.changeInterval = self.xmlFile:getValue("vehicle.baler.variableSpeedLimit#changeInterval", 1)
    spec.variableSpeedLimit.minSpeedLimit = self.xmlFile:getValue("vehicle.baler.variableSpeedLimit#minSpeedLimit", 5)
    spec.variableSpeedLimit.maxSpeedLimit = self.xmlFile:getValue("vehicle.baler.variableSpeedLimit#maxSpeedLimit", 15)
    spec.variableSpeedLimit.defaultSpeedLimit = self.xmlFile:getValue("vehicle.baler.variableSpeedLimit#defaultSpeedLimit", 10)
    ...
    spec.additives = {}
    spec.additives.fillUnitIndex = self.xmlFile:getValue("vehicle.baler.additives#fillUnitIndex")
    spec.additives.available = self:getFillUnitByIndex(spec.additives.fillUnitIndex) ~= nil
    spec.additives.usage = self.xmlFile:getValue("vehicle.baler.additives#usage", 0.0000275)
    ...
    if spec.hasUnloadingAnimation then
        if isUnloadingBale then
            if spec.unloadingState ~= Baler.UNLOADING_OPENING then
                ...
                if #spec.bales == 0 and spec.canUnloadUnfinishedBale and self:getFillUnitFillLevel(spec.fillUnitIndex) > spec.unfinishedBaleThreshold then
                    local fillTypeIndex = self:getFillUnitFillType(spec.fillUnitIndex)
...
function Baler:getCanUnloadUnfinishedBale()
    local spec = self.spec_baler
    return spec.canUnloadUnfinishedBale and self:getFillUnitFillLevel(spec.fillUnitIndex) > spec.unfinishedBaleThreshold
end

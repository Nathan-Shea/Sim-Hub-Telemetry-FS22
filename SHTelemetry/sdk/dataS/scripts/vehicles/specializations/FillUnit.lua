...
function FillUnit.initSpecialization()
    ...
    g_configurationManager:addConfigurationType("fillUnit", g_i18n:getText("configuration_fillUnit"), "fillUnit", nil, nil, nil, ConfigurationUtil.SELECTOR_MULTIOPTION)

    g_storeManager:addSpecType("capacity", "shopListAttributeIconCapacity", FillUnit.loadSpecValueCapacity, FillUnit.getSpecValueCapacity, "vehicle", {"fillUnit"})
    g_storeManager:addSpecType("fillTypes", "shopListAttributeIconFillTypes", FillUnit.loadSpecValueFillTypes, FillUnit.getSpecValueFillTypes, "vehicle", {"fillUnit"})
...
function FillUnit:onPostLoad(savegame)
    ...
    if self.isServer then
        -- fill units that do not have a start fill level will be only loaded from savegame if not reseted and savegame available
        ...
        if savegame ~= nil and savegame.xmlFile:hasProperty(savegame.key..".fillUnit") then
            ...
            while true do
                ...
                local allowLoading = fillUnitsToLoad[fillUnitIndex] == nil or -- always load (fill unit with start fill level)
                                    (fillUnitsToLoad[fillUnitIndex] ~= nil and not savegame.resetVehicles) -- only load if not reseted (fill unit without start fill level)
    ...
    if not self.isServer then
        ...
        for k, fillUnit in ipairs(self:getFillUnits()) do
            ...
            if fillUnit.canBeUnloaded and fillLevel > 0 and fillType.palletFilename ~= nil then
                table.insert(unloadingTasks, {fillUnitIndex=k, fillTypeIndex=fillTypeIndex, fillLevel=fillLevel, filename=fillType.palletFilename})
            end
        end
        ...
        local function unloadNext()
            ...
            if task ~= nil then
                ...
                if task.fillLevel > 0 then
                    local asyncCallback = function(_, vehicle, vehicleLoadState, arguments)
                    ...
                    local location = {x=x, y=y, z=z, yRot=place.rotY}
...
function FillUnit:addFillUnitFillLevel(farmId, fillUnitIndex, fillLevelDelta, fillTypeIndex, toolType, fillPositionData)
    ...
    if fillUnit ~= nil then
        ...
        if self.isServer then
            if spec.removeVehicleIfEmpty and fillUnit.fillLevel <= spec.removeVehicleThreshold then
        ...
        if appliedDelta ~= 0 then
            if fillUnit.fillLevelAnimationName ~= nil then
                if fillUnit.fillLevel > 0 or fillUnit.fillLevelAnimationResetOnEmpty then
...
function FillUnit:loadFillUnitFromXML(xmlFile, key, entry, index)
    ...
    if self.isClient then
        ...
        if self.loadDashboardsFromXML ~= nil then
            self:loadDashboardsFromXML(xmlFile, key, {valueTypeToLoad = "fillLevel",
                                                      valueObject = entry,
                                                      valueFunc = "fillLevel",
                                                      minFunc = 0,
                                                      maxFunc = "capacity"})

            self:loadDashboardsFromXML(xmlFile, key, {valueTypeToLoad = "fillLevelPct",
                                                      valueObject = entry,
            ...
            self:loadDashboardsFromXML(xmlFile, key, {valueTypeToLoad = "fillLevelWarning",
                                                      valueObject = entry,
                                                      valueFunc = "fillLevel",
                                                      minFunc = 0,
                                                      maxFunc = "capacity",
...
function FillUnit:updateFillUnitFillPlane(fillUnit)
    ...
    if fillPlane ~= nil then
        ...
        --assign new material on fill type change
...
function FillUnit:getIsFoldAllowed(superFunc, direction, onAiTurnOn)
    ...
    if not spec.allowFoldingWhileFilled then
        for fillUnitIndex, fillUnit in ipairs(spec.fillUnits) do
            if self:getFillUnitFillLevel(fillUnitIndex) > (fillUnit.allowFoldingThreshold or spec.allowFoldingThreshold) then
...
function FillUnit:getIsLevelerPickupNodeActive(superFunc, levelerNode)
    ...
    if fillLevelPct < levelerNode.minFillLevel or fillLevelPct > levelerNode.maxFillLevel then
        return false
    end
...
function FillUnit.loadSpecValueCapacity(xmlFile, customEnvironment, baseDir)
    ...
    if overwrittenCapacity ~= nil and overwrittenUnitText ~= nil then
        table.insert(fillUnitConfigurations, {{capacity=overwrittenCapacity, unit=overwrittenUnitText, conversionFunc=conversionFunc}})
    ...
    xmlFile:iterate(rootName..".fillUnit.fillUnitConfigurations.fillUnitConfiguration",function(_, key)
        ...
        xmlFile:iterate(key..".fillUnits.fillUnit", function(fillUnitIndex, fillUnitKey)
            ...
            if xmlFile:getValue(fillUnitKey.."#showCapacityInShop") ~= false and xmlFile:getValue(fillUnitKey.."#showInShop") ~= false then
                ...
                table.insert(fillUnitConfiguration, {capacity=capacity, unit=unitText, conversionFunc=conversionFunc, fillUnitIndex=fillUnitIndex})
            end
        end)
...
function FillUnit.loadSpecValueFillTypes(xmlFile, customEnvironment, baseDir)
    ...
    return {categoryNames=fillTypeCategoryNames, fillTypeNames=fillTypeNames, fruitTypeNames=fruitTypeNames, fruitTypeCategoryNames=fruitTypeCategoryNames, useWindrowed=useWindrowed, fillTypesByConfiguration=fillTypesByConfiguration}
end

...
function DynamicMountAttacher:onLoad(savegame)
    ...
    if self.isServer then
        self.xmlFile:iterate("vehicle.dynamicMountAttacher.mountCollisionMask", function(index, key)
            ...
            mountCollision.node = self.xmlFile:getValue(key.."#node", nil, self.components, self.i3dMappings)
            mountCollision.triggerNode = self.xmlFile:getValue(key.."#triggerNode", nil, self.components, self.i3dMappings)
            mountCollision.mountedCollisionMask = self.xmlFile:getValue(key.."#collisionMask")
        ...
        self.xmlFile:iterate("vehicle.dynamicMountAttacher.lockPosition", function(index, key)
            ...
            entry.jointNode = self.xmlFile:getValue(key .. "#jointNode", nil, self.components, self.i3dMappings)
...
function DynamicMountAttacher:updateDebugValues(values)
    ...
    if self.isServer then
        ...
        for object, _ in pairs(spec.pendingDynamicMountObjects) do
            table.insert(values, {name="pendingDynamicMountObject:", value=string.format("%s timeToMount: %d", object.configFileName or object, math.max(object.lastMoveTime + spec.dynamicMountAttacherTimeToMount - g_currentMission.time, 0))})
        end

        for object, _ in pairs(spec.dynamicMountedObjects) do
            table.insert(values, {name="dynamicMountedObjects:", value=string.format("%s", object.configFileName or object)})
        end
    end

...
function Attachable.initSpecialization()
    g_configurationManager:addConfigurationType("inputAttacherJoint", g_i18n:getText("configuration_inputAttacherJoint"), "attachable", nil, nil, nil, ConfigurationUtil.SELECTOR_MULTIOPTION)
        ...
        if self:getLastSpeed() > 0.25 or not self.finishedFirstUpdate then
            ...
            if (baseVehicle ~= nil or spec.steeringAxleReferenceComponentNode ~= nil) and (self.movingDirection >= 0 or spec.steeringAxleUpdateBackwards) then
                ...
                if spec.steeringAxleAngleScaleSpeedDependent then
                    ...
                    scale = MathUtil.clamp(1 + (self:getLastSpeed()-startSpeed) * 1.0/(startSpeed-endSpeed), 0, 1)
                end
        ...
        for i=1, #spec.supportAnimations do
            ...
            if animation.detachAfterAnimation then
                if not self:getIsAnimationPlaying(animation.animationName) then
                    doDetach = true
                elseif animation.detachAnimationTime < 1 then
...
function Attachable:loadInputAttacherJoint(xmlFile, key, inputAttacherJoint, index)
    ...
    if node ~= nil then
        ...
        inputAttacherJoint.jointOrigOffsetComponent = { localToLocal(self:getParentComponent(inputAttacherJoint.node), inputAttacherJoint.node, 0, 0, 0) }
        inputAttacherJoint.jointOrigDirOffsetComponent = { localDirectionToLocal(self:getParentComponent(inputAttacherJoint.node), inputAttacherJoint.node, 0, 0, 1) }
        ...
        if jointType == AttacherJoints.JOINTTYPE_IMPLEMENT then
            inputAttacherJoint.lowerRotLimitScale = { Utils.getNoNil(x, 0), Utils.getNoNil(y, 0), Utils.getNoNil(z, 1) }
        else
            inputAttacherJoint.lowerRotLimitScale = { Utils.getNoNil(x, 1), Utils.getNoNil(y, 1), Utils.getNoNil(z, 1) }
        end
...
function Attachable:loadSupportAnimationFromXML(supportAnimation, xmlFile, key)
    ...
    supportAnimation.delayedOnLoad = xmlFile:getValue(key.."#delayedOnLoad", false) -- defines if the animation is played onPostLoad or onLoadFinished -> useful if the animation collides e.g. with the folding animation

...
function Combine:onLoad(savegame)
    ...
    if self.loadDashboardsFromXML ~= nil then
        local dashboardData = {valueTypeToLoad = "workedHectars",
                               valueObject = spec,
...
function Combine:onUpdateTick(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
    ...
    if self.isServer then
        ...
        if self:getIsTurnedOn() then
            ...
            stats:updateStats("threshedTime", dt / (1000 * 60))
...
function Combine:loadCombineSetup(xmlFile, baseKey, entry)
    ...
    for _=1, inputBuffer.slotCount do
        table.insert(inputBuffer.buffer, {realArea = 0, area = 0, liters = 0, inputLiters = 0, strawRatio = 0, effectDensity = 0.2} )
    end
...
function Combine:startThreshing()
    ...
    if spec.numAttachedCutters > 0 then
        -- do not lower the cutter if the combine is turned on during headland (happens if ai was blocked during headland)

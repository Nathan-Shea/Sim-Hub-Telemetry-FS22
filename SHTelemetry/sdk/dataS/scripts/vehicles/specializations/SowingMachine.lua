...
function SowingMachine:onLoad(savegame)
    ...
    if self.isClient then
        ...
        spec.samples = {}
        spec.samples.work         = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.sowingMachine.sounds", "work", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
        spec.samples.airBlower    = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.sowingMachine.sounds", "airBlower", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
...
function SowingMachine:getFillUnitAllowsFillType(superFunc, fillUnitIndex, fillType)
    ...
    if spec.fillUnits[fillUnitIndex] ~= nil then
        if self:getFillUnitSupportsFillType(fillUnitIndex, fillType) then
            if fillType == FillType.SEEDS or spec.fillUnits[fillUnitIndex].fillType == FillType.SEEDS then
                return true
            end
        end
    end
...
function SowingMachine:onStateChange(state, data)
    if state == Vehicle.STATE_CHANGE_ATTACH or state == Vehicle.STATE_CHANGE_DETACH or Vehicle.STATE_CHANGE_FILLTYPE_CHANGE then
        ...
        if FillType.SEEDS ~= nil then
            ...
            FillUnit.addFillTypeSources(spec.fillTypeSources, root, self, {FillType.SEEDS})

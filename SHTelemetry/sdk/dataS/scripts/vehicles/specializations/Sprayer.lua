...
function Sprayer:onLoad(savegame)
    ...
    if self.isClient then
        ...
        spec.samples = {}
        spec.samples.work = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.sprayer.sounds", "work", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
        spec.samples.spray = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.sprayer.sounds", "spray", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
        ...
        if self.addAITerrainDetailProhibitedRange ~= nil then
            ...
            if sprayTypeDesc ~= nil then
                if sprayTypeDesc.isHerbicide then
                    ...
                    -- TODO enable after field crops query support bitVectorMaps
                ...
                if sprayTypeDesc.isHerbicide or sprayTypeDesc.isFertilizer then
                    ...
                    if fruitType.terrainDataPlaneId ~= nil and fruitType.name:lower() ~= "grass" then
...
function Sprayer:loadSprayTypeFromXML(xmlFile, key, sprayType)
    ...
    sprayType.samples = {}
    sprayType.samples.work = g_soundManager:loadSampleFromXML(xmlFile, key.. ".sounds", "work", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
    sprayType.samples.spray = g_soundManager:loadSampleFromXML(xmlFile, key.. ".sounds", "spray", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
    ...
    sprayType.ai = {}
    sprayType.ai.leftMarker = self.xmlFile:getValue(key .. ".ai.areaMarkers#leftNode", nil, self.components, self.i3dMappings)
    sprayType.ai.rightMarker = self.xmlFile:getValue(key .. ".ai.areaMarkers#rightNode", nil, self.components, self.i3dMappings)
    sprayType.ai.backMarker = self.xmlFile:getValue(key .. ".ai.areaMarkers#backNode", nil, self.components, self.i3dMappings)

    sprayType.ai.sizeLeftMarker = self.xmlFile:getValue(key .. ".ai.sizeMarkers#leftNode", nil, self.components, self.i3dMappings)
    sprayType.ai.sizeRightMarker = self.xmlFile:getValue(key .. ".ai.sizeMarkers#rightNode", nil, self.components, self.i3dMappings)
    sprayType.ai.sizeBackMarker = self.xmlFile:getValue(key .. ".ai.sizeMarkers#backNode", nil, self.components, self.i3dMappings)
    ...
    sprayType.usageScale.workingWidth = self.xmlFile:getValue(key..".usageScales#workingWidth", 12)
    sprayType.usageScale.workAreaIndex = self.xmlFile:getValue(key..".usageScales#workAreaIndex")
...
function Sprayer:getAIMarkers(superFunc)
    ...
    local sprayType = self:getActiveSprayType()
    if sprayType ~= nil and not spec.useAttributesOfAttachedImplement then
        if sprayType.ai.rightMarker ~= nil then
...
function Sprayer:getAISizeMarkers(superFunc)
    local sprayType = self:getActiveSprayType()
    if sprayType ~= nil then
        if sprayType.ai.sizeLeftMarker ~= nil then
    ...
    if self.isClient then
        ...
        if spec.needsToBeFilledToTurnOn then
            if self:getIsActiveForInput() and self:getIsSelected() and not self.isAlwaysTurnedOn then
                if not self:getCanBeTurnedOn() and self:getFillUnitFillLevel(self:getSprayerFillUnitIndex()) <= 0 and self:getFillUnitCapacity(self:getSprayerFillUnitIndex()) > 0 then
                    return true
                end
            end
        end
...
function Sprayer:onStartWorkAreaProcessing(dt)
    ...
    if isExternallyFilled then
        -- only consume while ai is on field, not while turning, so we check for the tools turned on state
        if self:getIsTurnedOn() then
...
function Sprayer:onEndWorkAreaProcessing(dt, hasProcessed)
    ...
    if self.isServer then
        if spec.workAreaParameters.isActive then
            ...
            stats:updateStats("sprayedTime", dt/(1000*60))

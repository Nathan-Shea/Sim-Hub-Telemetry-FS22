...
function AnimatedVehicle:loadAnimation(xmlFile, key, animation, components)
    ...
    if name ~= nil then
        ...
        if self.isClient then
            ...
            animation.eventSamples.stopTimePos = {}
            animation.eventSamples.stopTimeNeg = {}
    ...
    self:registerAnimationValueType("shaderParameter", "shaderStartValues", "shaderEndValues", false, AnimationValueFloat,
        ...
        function(value, x, y, z, w)
            if value.customShaderParameterMask then
                ...
                if value.shaderParameterMask[1] == 0 then x = sx end
                if value.shaderParameterMask[2] == 0 then y = sy end
                if value.shaderParameterMask[3] == 0 then z = sz end
                if value.shaderParameterMask[4] == 0 then w = sw end
            end

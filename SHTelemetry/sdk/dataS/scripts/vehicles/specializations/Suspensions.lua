...
function Suspensions:onLoad(savegame)
    if self.isClient then
        ...
        while true do
            ...
            entry.refNodeOffset = {0, 0, 0}
            if entry.node ~= nil then
                local component = self:getParentComponent(entry.node)
                if component ~= nil then
                    entry.component = component
                    entry.refNodeOffset = {localToLocal(entry.node, component, 0, 0, 0)}
                end
            end
            ...
            if (entry.node ~= nil and entry.component ~= nil) or entry.useCharacterTorso then
                ...
                if not entry.isRotational and not entry.useCharacterTorso then
                    entry.baseTranslation = {getTranslation(entry.node)}
                ...
                entry.suspensionParameters[1] = {}
                entry.suspensionParameters[2] = {}
                entry.suspensionParameters[3] = {}
                for j=1,2 do
                ...
                entry.curRotation = {0, 0, 0}
                entry.curRotationSpeed = {0, 0, 0}

                entry.curTranslation = {0, 0, 0}
                entry.curTranslationSpeed = {0, 0, 0}

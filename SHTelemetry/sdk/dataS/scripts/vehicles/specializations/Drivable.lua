...
function Drivable:onLoad(savegame)
    ...
    if self.loadDashboardsFromXML ~= nil then
        ...
        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "cruiseControl",
                                                           valueObject = spec.cruiseControl,
                                                           valueFunc = "speed"})

        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "directionForward",
                                                           valueObject = self,
                                                           valueFunc = "getIsDrivingForward"})

        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "directionBackward",
                                                           valueObject = self,
                                                           valueFunc = "getIsDrivingBackward"})

        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "movingDirection",
                                                           valueObject = self,
                                                           valueFunc = "getDrivingDirection",
                                                           minFunc = -1,
                                                           maxFunc = 1})

        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "cruiseControlActive",
                                                           valueObject = spec.cruiseControl,
                                                           valueFunc = "state",
                                                           valueCompare = Drivable.CRUISECONTROL_STATE_ACTIVE})

        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "accelerationAxis",
                                                           valueObject = self,
                                                           valueFunc = "getAccelerationAxis"})

        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "decelerationAxis",
                                                           valueObject = self,
                                                           valueFunc = "getDecelerationAxis"})

        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "ac_decelerationAxis",
                                                           valueObject = self,
                                                           valueFunc = "getAcDecelerationAxis"})

        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "steeringAngle",
                                                           valueObject = self,
                                                           valueFunc = "getDashboardSteeringAxis",
                                                           minFunc = -1,
                                                           maxFunc = 1})
        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "combinedPedalLeft",
                                                           valueObject = self,
                                                           valueFunc = Drivable.getDashboardCombinedPedalLeft,
                                                           minFunc = -1,
                                                           maxFunc = 1})
        self:loadDashboardsFromXML(self.xmlFile, dashKey, {valueTypeToLoad = "combinedPedalRight",
                                                           valueObject = self,
...
-- @return bool isActive is cruise control active

...
function PowerConsumer.initSpecialization()
    g_configurationManager:addConfigurationType("powerConsumer", g_i18n:getText("configuration_powerConsumer"), "powerConsumer", nil, nil, nil, ConfigurationUtil.SELECTOR_MULTIOPTION)
...
-- @param bool expected expected torque
...
function PowerConsumer:consoleSetPowerConsumer(neededMinPtoPower, neededMaxPtoPower, forceFactor, maxForce, forceDir, ptoRpm)
    ...
    if g_currentMission ~= nil and g_currentMission.controlledVehicle ~= nil then
        if g_currentMission.controlledVehicle:getSelectedImplement() ~= nil and g_currentMission.controlledVehicle:getSelectedImplement().object.spec_powerConsumer ~= nil then

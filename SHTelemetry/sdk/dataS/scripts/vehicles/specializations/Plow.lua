...
function Plow.initSpecialization()
    g_configurationManager:addConfigurationType("plow", g_i18n:getText("configuration_design"), "plow", nil, nil, nil, ConfigurationUtil.SELECTOR_MULTIOPTION)
...
function Plow:onLoad(savegame)
    ...
    if self.isClient then
        spec.samples = {}
        spec.samples.turn = g_soundManager:loadSampleFromXML(self.xmlFile, configKey .. ".sounds", "turn", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
...
function Plow:setRotationMax(rotationMax, noEventSend, turnAnimationTime)
    if noEventSend == nil or noEventSend == false then
        if g_server ~= nil then
            g_server:broadcastEvent(PlowRotationEvent.new(self, rotationMax), nil, nil, self)
        else
...
function Plow:onEndWorkAreaProcessing(dt)
    ...
    if self.isServer then
        ...
        if spec.isWorking then
            stats:updateStats("plowedTime", dt/(1000*60))
        end
    end
...
function Plow:onRootVehicleChanged(rootVehicle)
    ...
    if #spec.foldingParts > 0 then
        ...
        if actionController ~= nil then
            ...
            spec.controlledActionRotate:setFinishedFunctions(self, function(vehicle) return not vehicle.spec_foldable.rotationMax end, false, false)
...
---
function Plow.actionControllerRotateEvent(self, direction)
    local spec = self.spec_plow
    if spec.rotationPart.turnAnimation ~= nil then
        if self:getIsPlowRotationAllowed() then
            if spec.rotationMax then

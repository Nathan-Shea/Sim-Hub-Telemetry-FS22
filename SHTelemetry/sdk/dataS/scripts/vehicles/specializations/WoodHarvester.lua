...
function WoodHarvester:onLoad(savegame)
    ...
    if self.loadDashboardsFromXML ~= nil then
        self:loadDashboardsFromXML(self.xmlFile, "vehicle.woodHarvester.dashboards", {valueTypeToLoad = "cutLength",
                                                                                      valueObject = spec,
                                                                                      valueFunc = function()
        ...
        self:loadDashboardsFromXML(self.xmlFile, "vehicle.woodHarvester.dashboards", {valueTypeToLoad = "curCutLength",
                                                                                      valueObject = spec,
                                                                                      valueFunc = function()
                                                                                          return math.abs(spec.attachedSplitShapeStartY - spec.attachedSplitShapeY) * 100
                                                                                      end})

        self:loadDashboardsFromXML(self.xmlFile, "vehicle.woodHarvester.dashboards", {valueTypeToLoad = "diameter",
                                                                                      valueObject = spec,
                                                                                      valueFunc = function()
...
function WoodHarvester:onStateChange(state, data)
    if self.isServer then
        if state == Vehicle.STATE_CHANGE_MOTOR_TURN_ON then

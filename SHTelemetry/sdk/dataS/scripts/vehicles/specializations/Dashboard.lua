...
function Dashboard:loadDashboardFromXML(xmlFile, key, dashboard, dashboardData)
    ...
    elseif dashboard.displayTypeIndex == Dashboard.TYPES.MULTI_STATE then
        if not self:loadMultiStateDashboardFromXML(xmlFile, key, dashboard) then
            return false
        end
    end
...
--            if getMaterialCustomShaderFilename(materialId):contains("glowShader.xml")
--            and getMaterialCustomShaderVariation(materialId) == "staticLight" then
--                if not getMaterialNormalMapFilename(materialId):contains("default_normal") then
--                    Logging.xmlWarning(self.xmlFile, "Glow shader mesh using different normal map than default! '%s' '%s' '%s'", getName(node), getMaterialNormalMapFilename(materialId), key)
--                end
--            end
--
--            if getMaterialCustomShaderFilename(materialId):contains("glowShader.xml")
--            and getMaterialCustomShaderVariation(materialId) == "staticLight" then
--                if not getMaterialGlossMapFilename(materialId):contains("default_vmask") and not getMaterialGlossMapFilename(materialId):contains("singleColorTexture") then
--                    Logging.xmlWarning(self.xmlFile, "Glow shader mesh using different specular map than default! '%s' '%s' '%s'", getName(node), getMaterialGlossMapFilename(materialId), key)
--                end
--            end
...
function Dashboard:loadNumberDashboardFromXML(xmlFile, key, dashboard)
    ...
    if dashboard.numbers ~= nil then
        ...
        XMLUtil.checkDeprecatedXMLElements(xmlFile, self.configFileName, key.."#hiddenAlpha") --FS19 to FS22
        ...
        dashboard.maxValue = (10 ^ (dashboard.numChilds)) - 1/(10^dashboard.precision) -- e.g. max with 2 childs and 1 float -> 10^2 - 1/10 -> 99.9 -> makes sure that display doesn't show 00.0 if value is 100
    else
...
function Dashboard:loadTextDashboardFromXML(xmlFile, key, dashboard)
    ...
    if dashboard.node ~= nil then
        ...
        if dashboard.fontMaterial ~= nil then
            dashboard.characterLine = dashboard.fontMaterial:createCharacterLine(dashboard.node,
                                                                                 dashboard.textMask:len(),
                                                                                 dashboard.textSize,
                                                                                 dashboard.textColor,
                                                                                 dashboard.hiddenColor,
                                                                                 dashboard.emissiveScale,
                                                                                 dashboard.textScaleX,
                                                                                 dashboard.textScaleY,
                                                                                 dashboard.textAlignment,
                                                                                 nil,
                                                                                 dashboard.fontThickness)
...
function Dashboard:loadMultiStateDashboardFromXML(xmlFile, key, dashboard)
    ...
    dashboard.set = function(x, y, z, rx, ry, rz, sx, sy, sz, vis)
    ...
    if type(newValue) == "number" then
        ...
        for i=1, #dashboard.numberNodes do
            ...
            if value > 0 then
                ...
                if not (isActive and i - 1 <= dashboard.precision) then
                    setVisibility(numberNode, false)
                end
            end
        end
...
function Dashboard:defaultTextDashboardStateFunc(dashboard, newValue, minValue, maxValue, isActive)
    if type(newValue) == "number" then
        ...
        local value = string.format(dashboard.textFormatStr, int, math.abs(math.floor((floatPart + 0.000001) * (10 ^ dashboard.textFormatPrecision))))
...
function Dashboard:defaultRotationDashboardStateFunc(dashboard, newValue, minValue, maxValue, isActive)
    ...
    if type(newValue) == "boolean" then
        ...
        if dashboard.minValueRot ~= nil and dashboard.maxValueRot ~= nil then
    ...
    if dashboard.node ~= nil then
        ...
        if activeState ~= dashboard.lastState then
            ...
            if dashboard.doInterpolation then
                ...
                local interpolator = ValueInterpolator.new(dashboard.node.."_dashboard", dashboard.get, dashboard.set, {translation[1], translation[2], translation[3],
                ...
                if interpolator ~= nil then
                    ...
                    dashboard.interpolator:setFinishedFunc(function(dash) dash.interpolator = nil end, dashboard)
                end
            else
...
function Dashboard:warningState(dashboard, newValue, minValue, maxValue, isActive)
    Dashboard.defaultDashboardStateFunc(self, dashboard, newValue > dashboard.warningThresholdMin and newValue < dashboard.warningThresholdMax, minValue, maxValue, isActive)
end
...
function Dashboard.registerDashboardXMLPaths(schema, basePath, availableValueTypes)
    ...
    schema:register(XMLValueType.FLOAT, basePath .. ".dashboard(?)#fontThickness", "(TEXT) Thickness factor for font characters", 1.0)

...
function Mulcher:onLoad(savegame)
    ...
    if self.isClient then
        spec.samples = {}
        spec.samples.work = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.mulcher.sounds", "work", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
    ...
    if self.addAIGroundTypeRequirements ~= nil then
        ...
        for _, fruitType in ipairs(g_fruitTypeManager:getFruitTypes()) do
            if fruitType.destruction.canBeDestroyed then
                if fruitType.mulcher.state > fruitType.cutState then

...
function Enterable:onLoad(savegame)
    ...
    while true do
        ...
        if node ~= nil then
            ...
            setReflectionMapObjectMasks(node, 32896, 2147483648, true) --0x8080, 0x80000000
    ...
    if self.loadDashboardsFromXML ~= nil then
        self:loadDashboardsFromXML(self.xmlFile, "vehicle.enterable.dashboards", {valueTypeToLoad = "time",
                                                                                  valueObject = g_currentMission.environment,
                                                                                  valueFunc = "getEnvironmentTime"})

        self:loadDashboardsFromXML(self.xmlFile, "vehicle.enterable.dashboards", {valueTypeToLoad = "timeHours",
                                                                                  valueObject = g_currentMission.environment,
                                                                                  valueFunc = function(env) local time = env:getEnvironmentTime(); return math.floor(time) + time % 1 * 100 / 60 end})

        self:loadDashboardsFromXML(self.xmlFile, "vehicle.enterable.dashboards", {valueTypeToLoad = "timeMinutes",
                                                                                  valueObject = g_currentMission.environment,
                                                                                  valueFunc = function(env) return env:getEnvironmentTime() % 1 * 100 end})

        self:loadDashboardsFromXML(self.xmlFile, "vehicle.enterable.dashboards", {valueTypeToLoad = "operatingTime",
                                                                                  valueObject = self,
                                                                                  valueFunc = "getFormattedOperatingTime"})

        self:loadDashboardsFromXML(self.xmlFile, "vehicle.enterable.dashboards", {valueTypeToLoad = "outsideTemperature",
        ...
        for _, vehicle in ipairs(vehicles) do
            ...
            if vehicleSpec ~= nil then
                if vehicle ~= self then
                    if vehicleSpec.isEntered or vehicleSpec.isControlled then
    ...
    if visible then
        ...
        for _, mirror in pairs(spec.mirrors) do
            ...
            if getEffectiveVisibility(mirror.parentNode) and getIsInCameraFrustum(mirror.node, spec.activeCamera.cameraNode, g_presentedScreenAspectRatio) then
                ...
                --local dirX, dirY, dirZ = worldToLocal(spec.activeCamera.cameraNode, getWorldTranslation(mirror.node))
...
---
function Enterable.actionEventCameraZoomIn(self, actionName, inputValue, callbackState, isAnalog, isMouse)
    local spec = self.spec_enterable
    local offset = -0.2
    if isMouse then -- mouse wheel zoom, must use larger value because of engine input handling
...
---
function Enterable.actionEventCameraZoomOut(self, actionName, inputValue, callbackState, isAnalog, isMouse)
    local spec = self.spec_enterable
    local offset = 0.2
    if isMouse then -- mouse wheel zoom, must use larger value because of engine input handling
        offset = offset * InputBinding.MOUSE_WHEEL_INPUT_FACTOR

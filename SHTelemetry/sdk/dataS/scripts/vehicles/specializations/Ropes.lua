...
function Ropes:onLoad(savegame)
    ...
    if self.isClient then
        ...
        while true do
            ...
            local entry = {}
            entry.baseNode = self.xmlFile:getValue(key .. "#baseNode", nil, self.components, self.i3dMappings)
            entry.targetNode = self.xmlFile:getValue(key .. "#targetNode", nil, self.components, self.i3dMappings)
            entry.baseParameters = self.xmlFile:getValue(key .. "#baseParameters", nil, true)
            entry.targetParameters = self.xmlFile:getValue(key .. "#targetParameters", nil, true)

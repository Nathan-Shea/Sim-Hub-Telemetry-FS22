...
function TreeSaplingPallet.initSpecialization()
    g_configurationManager:addConfigurationType("treeSaplingType", g_i18n:getText("configuration_treeType"), "treeSaplingPallet", nil, nil, nil, ConfigurationUtil.SELECTOR_MULTIOPTION)
    ...
    if spec.treeTypeFilename ~= nil then
        ...
        self.xmlFile:iterate(nodeKey, function(_, key)
            ...
            if entry.node ~= nil then
                ...
                entry.sharedLoadRequestId = self:loadSubSharedI3DFile(treeTypeFilename, false, false, self.onTreeSaplingLoaded, self, {entry=entry})

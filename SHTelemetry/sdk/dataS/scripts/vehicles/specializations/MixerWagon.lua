...
function MixerWagon:onLoad(savegame)
    ...
    -- load in a trigger for displaying mixer wagon HUDExtension if player is near
...
---Trigger callback function adding mixer wagon hud extension to UI if player or vehicle is near the mixer wagon
function MixerWagon:onHUDTriggerCallback(triggerId, otherActorId, onEnter, onLeave, onStay, otherShapeId)
    ...
    if object ~= nil and object:isa(Vehicle) and not SpecializationUtil.hasSpecialization(Enterable, object.specializations) then
        -- only enterable vehicles are relevant
        return
    ...
    if object ~= nil and object ~= self then  -- exclude mixer wagon from triggering on himself
        local spec = self.spec_mixerWagon
        if onEnter then
            if not g_currentMission.accessHandler:canPlayerAccess(self) then
                return
            end
            -- count number of nodes per vehicle inside trigger
        ...
        if spec.vehicleToNodeCount[object] == 1 then
            -- only add once on inital node enter

...
function Dischargeable.initSpecialization()
    g_configurationManager:addConfigurationType("dischargeable", g_i18n:getText("configuration_dischargeable"), "dischargeable", nil, nil, nil, ConfigurationUtil.SELECTOR_MULTIOPTION)
...
function Dischargeable:onLoad(savegame)
    ...
    if self.loadDashboardsFromXML ~= nil then
        self:loadDashboardsFromXML(self.xmlFile, "vehicle.dischargeable.dashboards", {valueTypeToLoad = "activeDischargeNode",
                                                                                      valueObject = self,
        ...
        self:loadDashboardsFromXML(self.xmlFile, "vehicle.dischargeable.dashboards", {valueTypeToLoad = "dischargeState",
                                                                                      valueObject = spec,
            ...
            if trigger.numObjects > 0 then
                ...
                for object, data in pairs(trigger.objects) do
                    ...
                    if dischargeNode.dischargeFailedReason == Dischargeable.DISCHARGE_REASON_FILLTYPE_NOT_SUPPORTED
                        or dischargeNode.dischargeFailedReason == Dischargeable.DISCHARGE_REASON_NO_FREE_CAPACITY
                        or dischargeNode.dischargeFailedReason == Dischargeable.DISCHARGE_REASON_NO_ACCESS then
                    ...
                    if dischargeNode.dischargeFailedReason ~= nil then
                        ...
                        if dischargeNode.dischargeFailedReason ~= Dischargeable.DISCHARGE_REASON_FILLTYPE_NOT_SUPPORTED then
...
function Dischargeable:loadDischargeNode(xmlFile, key, entry)
    ...
    entry.raycast = {}
    entry.raycast.node = xmlFile:getValue(key .. ".raycast#node", entry.node, self.components, self.i3dMappings)
        ...
        if validObject then
            if object.getFillUnitIndexFromNode ~= nil then
                ...
                if fillUnitIndex ~= nil then
                    ...
                    dischargeNode.dischargeHitObject = object -- hit object, no matter if it's fillable or not
    ...
    if self:getCanToggleDischargeToGround() then
        ...
        if spec.currentDischargeState == Dischargeable.DISCHARGE_STATE_OFF then
            if self:getCanDischargeToGround(currentDischargeNode) then
                ...
                if not self:getCanDischargeToLand(currentDischargeNode) then
...
function Dischargeable.updateActionEvents(self)
    ...
    if spec.currentDischargeState == Dischargeable.DISCHARGE_STATE_OFF then
        ...
        if self:getIsDischargeNodeActive(currentDischargeNode) then
            ...
            elseif self:getCanDischargeToGround(currentDischargeNode) and self:getCanToggleDischargeToGround() then
...
---
function Dischargeable.dashboardCurrentDischargeNodeState(self, dashboard, newValue, minValue, maxValue, isActive)
...
---
function Dischargeable.dashboardDischargeActiveState(self, dashboard, newValue, minValue, maxValue, isActive)

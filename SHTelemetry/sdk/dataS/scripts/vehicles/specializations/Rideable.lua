...
function Rideable:updateDirt()
    ...
    if cluster ~= nil and cluster.getDirtFactor ~= nil then
        ...
        I3DUtil.setShaderParameterRec(spec.meshNode, "RDT", nil, dirtFactor, nil, nil)
    end
end
...
function Rideable:onUpdate(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
    ...
    if spec.playerToEnter ~= nil and spec.checkPlayerToEnter then
        if spec.playerToEnter == g_currentMission.player then
    ...
    if self.isServer and spec.doHusbandryCheck > 0 then
        ...
        if isInRange then
            ...
            if husbandry ~= nil then
                ...
                isInStable = true
--                 g_currentMission:addIngameNotification(FSBaseMission.INGAME_NOTIFICATION_OK, string.format(g_i18n:getText("ingameNotification_horseInStable"), spec.cluster:getName()))
            else
--                 g_currentMission:addIngameNotification(FSBaseMission.INGAME_NOTIFICATION_CRITICAL, string.format(g_i18n:getText("ingameNotification_horseNotInStable"), spec.cluster:getName()))
                isInStable = false
            end
...
function Rideable:onUpdateInterpolation(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
    ...
    if spec.isOnGround and (math.abs(spec.currentSpeed) > 0.001 or math.abs(spec.currentTurnSpeed) > 0.001) then
        ...
        raycastClosest(fx, fy + Rideable.GROUND_RAYCAST_OFFSET, fz, 0.0, -1.0, 0.0, "groundRaycastCallback", Rideable.GROUND_RAYCAST_MAXDISTANCE, self, Rideable.GROUND_RAYCAST_COLLISIONMASK)
        ...
        raycastClosest(bx, by + Rideable.GROUND_RAYCAST_OFFSET, bz, 0.0, -1.0, 0.0, "groundRaycastCallback", Rideable.GROUND_RAYCAST_MAXDISTANCE, self, Rideable.GROUND_RAYCAST_COLLISIONMASK)
        ...
        raycastClosest(posX, posY, posZ, 0.0, -1.0, 0.0, "groundRaycastCallback", Rideable.GROUND_RAYCAST_MAXDISTANCE, self, Rideable.GROUND_RAYCAST_COLLISIONMASK)
...
function Rideable:jump()
    ...
    local velY = math.sqrt(-2.0 * spec.gravity * spec.jumpHeight)
...
function Rideable:updateKinematic(dt)
    ...
    local maxTurnSpeedChange = (MathUtil.clamp((fastestSpeed - spec.currentSpeed) / (fastestSpeed - slowestSpeed), 0, 1) * 0.4 + 0.8) -- Use smaller changes when walking slowly (between 0.8 and 1.2 rad/s^2)
...
function Rideable:onVehicleCharacterChanged(character)
    if self.isClient then
        ...
        if self:getIsControlled() then
            if g_currentMission.hud.fadeScreenElement:getAlpha() > 0.0 then
...
-- @param bool isAnalog true if input is analog
...
function Rideable:updateFootsteps(dt, speed)
    ...
    if speed > epsilon then
        ...
        for k, hoofInfo in pairs(spec.hooves) do
            ...
            raycastClosest(posX, posY + Rideable.GROUND_RAYCAST_OFFSET, posZ, 0.0, -1.0, 0.0, "groundRaycastCallback", Rideable.GROUND_RAYCAST_MAXDISTANCE, self, Rideable.GROUND_RAYCAST_COLLISIONMASK)
            ...
            -- DebugUtil.drawDebugNode(hoofInfo.node, string.format("[%s] (%.6f/%.6f|%s)", getName(hoofInfo.node), posY, terrainY, tostring(((posY - terrainY) < 0.05))))

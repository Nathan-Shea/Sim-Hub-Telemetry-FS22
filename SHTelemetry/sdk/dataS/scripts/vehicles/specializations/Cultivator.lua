...
function Cultivator:onLoad(savegame)
    ...
    if self.isClient then
        spec.samples = {}
        spec.samples.work = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.cultivator.sounds", "work", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
...
function Cultivator:onEndWorkAreaProcessing(dt)
    ...
    if self.isServer then
        ...
        if spec.isWorking then
            stats:updateStats("cultivatedTime", dt/(1000*60))
        end
    end
...
function Cultivator:onStateChange(state, data)
    ...
    if self.isServer then
        if state == Vehicle.STATE_CHANGE_TURN_ON or state == Vehicle.STATE_CHANGE_TURN_OFF then
            local spec = self.spec_cultivator

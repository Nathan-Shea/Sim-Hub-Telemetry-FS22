...
function AutoLoader:onLoad(savegame)
    if self.isServer then
        ...
        while true do
            ...
            local entry = {}
            entry.node = self.xmlFile:getValue(placeKey .. "#node", nil, self.components, self.i3dMappings)
...
function AutoLoader:getFirstValidLoadPlace()
    ...
    for i=1, #spec.loadPlaces do
        ...
        overlapBox(x + offsetX, y + offsetY + offsetZ, z, rx, ry, rz, sizeX / 2, sizeY / 2, sizeZ / 2, "autoLoaderOverlapCallback", self, 3212828671, true, false, true)

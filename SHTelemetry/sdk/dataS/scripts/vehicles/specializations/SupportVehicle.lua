...
function SupportVehicle:onLoad(savegame)
    ...
    spec.heightChecks = {}
    table.insert(spec.heightChecks, {x= self.size.width / 2 + self.size.widthOffset, z= self.size.length / 2 + self.size.lengthOffset})
    table.insert(spec.heightChecks, {x=-self.size.width / 2 + self.size.widthOffset, z= self.size.length / 2 + self.size.lengthOffset})
    table.insert(spec.heightChecks, {x= self.size.width / 2 + self.size.widthOffset, z=-self.size.length / 2 + self.size.lengthOffset})
    table.insert(spec.heightChecks, {x=-self.size.width / 2 + self.size.widthOffset, z=-self.size.length / 2 + self.size.lengthOffset})
...
function SupportVehicle:addSupportVehicle(filename, inputAttacherJointIndex, attacherJointIndex)
    ...
    if spec.filename ~= nil then
        if spec.supportVehicle == nil then
            -- make sure below the vehicle is enough space to place the support vehicle
            ...
            if storeItem ~= nil then
                ...
                if inputAttacherJoint ~= nil then
                    ...
                    local vehicle = VehicleLoadingUtil.loadVehicle(storeItem.xmlFilename, location, false, 0, Vehicle.PROPERTY_STATE_NONE, self:getActiveFarm(), spec.configurations, nil, SupportVehicle.supportVehicleLoaded, self, {attacherJointIndex, inputAttacherJointIndex, inputAttacherJoint.node})
                    if vehicle ~= nil then
...
function SupportVehicle:supportVehicleLoaded(vehicle, vehicleLoadState, asyncCallbackArguments)
    if vehicleLoadState == VehicleLoadingUtil.VEHICLE_LOAD_OK and vehicle ~= nil then
        ...
        -- check again if vehicle is already attached (async loading of support vehicle can have a delay
        self:addToPhysics()
        ...
        if not self.isDeleted and attacherVehicle == nil then
            ...
            vehicle:attachImplement(self, asyncCallbackArguments[2], asyncCallbackArguments[1], true, nil, nil, true)

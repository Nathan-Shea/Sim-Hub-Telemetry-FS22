...
function AIDriveStrategy:getDistanceToEndOfField(dt, vX,vY,vZ)
    ...
    -- implement list must be sorted in decreasing order by distance to vehicle (front to back)
    local lookAheadDist
    ...
    for i=1, #attachedAIImplements do
        ...
        if i == 1 then
            ...
            -- tools behind the first tool should test at the same 'z value' if they are the same type
            -- e.g. two cutters in comparison to fruit preparer and harvester
        ...
        if VehicleDebug.state == VehicleDebug.DEBUG_AI then
            ...
            self.vehicle:addAIDebugLine({lX,lY,lZ}, {rX,rY,rZ}, {0,1,0})
            self.vehicle:addAIDebugLine({lX,lY,lZ}, {hX,hY,hZ}, {0,1,0})
        end

        -- We're good if any tool is above a field owned by the player and has valid ground
        if area > 0 then
            local farmId = self.vehicle:getAIJobFarmId()

            local posX, posZ = (rX + hX)*0.5, (rZ + hZ)*0.5
            if g_currentMission.accessHandler:canFarmAccessLand(farmId, posX, posZ)
                or g_missionManager:getIsMissionWorkAllowed(farmId, posX, posZ, nil) then
                hasField = true
                break
            else
                ownedField = false

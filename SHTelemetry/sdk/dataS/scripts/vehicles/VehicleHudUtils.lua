...
function VehicleHudUtils.loadHud(vehicle, xmlFile, name, baseName, index)
    ...
    while true do
        ...
        local entry = {}
        entry.numbers = I3DUtil.indexToObject(vehicle.components, getXMLString(xmlFile, key.."#numbers"), vehicle.i3dMappings)
        ...
        if entry.numbers ~= nil or entry.animName ~= nil then
            ...
            if entry.numbers ~= nil then
                ...
                entry.maxValue = (10 ^ (entry.numChilds)) - 1/(10^entry.precision) -- e.g. max with 2 childs and 1 float -> 10^2 - 1/10 -> 99.9 -> makes sure that display doesn't show 00.0 if value is 100
            end

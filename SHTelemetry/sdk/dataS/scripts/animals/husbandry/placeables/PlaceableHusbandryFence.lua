...
function PlaceableHusbandryFence:onLoad(savegame)
    ...
    self.xmlFile:iterate("placeable.husbandry.fences.fence", function(_, key)
        ...
        local fence = {}
        fence.filename = Utils.getFilename(filename, self.baseDirectory)
        fence.nodes = {}
        fence.parts = {}
        fence.rootNode = createTransformGroup("fence")
    ...
    self.xmlFile:iterate("placeable.husbandry.fences.gate", function(_, key)
        ...
        local gate = {}
        gate.filename = Utils.getFilename(filename, self.baseDirectory)
        gate.node = self.xmlFile:getValue(key .. "#node", nil, self.components, self.i3dMappings)
...
function PlaceableHusbandryFence:onGateI3DLoaded(i3dNode, failedReason, args)
    ...
    if i3dNode ~= 0 then
        ...
        if gateRootNode ~= nil then
            ...
            local saveId = "test" --string.format("AnimatedObject_%s_gate_%d_%d_%d_%d", self.configFileName, segment.x1, segment.z1, segment.x2, segment.x2)
...
function PlaceableHusbandryFence:createHusbandryFence(fence, poleRoot, panelRoot, panelLength)
    ...
    for k, node in ipairs(fence.nodes) do
        ...
        if nextNode ~= nil then
            ...
            setRotation(pole, 0, rotY, 0)
            setTranslation(pole, posX, posY, posZ)
            table.insert(fence.parts, {pole=pole})
        end
    end
        ...
        if lastPart ~= nil then
            if lastPart.panel ~= nil then
                ...
                if updateCollision then
                    ...
                    local xDir, yDir, zDir = 0, yDif, lastPart.length

...
function PlaceableHusbandryAnimals:onLoad(savegame)
    ...
    if spec.maxNumAnimals > maxAnimals then
        Logging.xmlWarning(xmlFile, "Maximum number of animals reached! Maximum is '%d'!", maxAnimals)
        spec.maxNumAnimals = maxAnimals
    end

    if GS_IS_MOBILE_VERSION then
        spec.maxNumVisualAnimals = math.min(spec.maxNumVisualAnimals, 8) -- limit to 8 max on mobile
    end
    ...
    if maxNumVisualAnimals ~= nil then
        if maxNumVisualAnimals > spec.maxNumAnimals then
            maxNumVisualAnimals = spec.maxNumAnimals
        end

        if maxNumVisualAnimals > spec.maxNumVisualAnimals then
    ...
    if not self.isServer then
        ...
        if cluster ~= nil then
            ...
            if x ~= nil then
                ...
                local filename = cluster:getRidableFilename()
                local location = {x=x, z=z, yRot=ry}
                local arguments = {
                    player = player,
                    cluster = cluster
                }

...
-- @param int xmlFile Sample definition XML file handle
...
-- @param bool is2D If true, the sample is interpreted as a non-spatial sound sample
...
-- @return bool True if the definition and parameters are valid
-- @return bool True if an external XML file is loaded in place of the given parameter, caller must delete the handle afterwards!
-- @return int XML file handle, either the one passed in as an argument or an alternate external sound file definition which must be released by the caller
...
-- @return int Link node target for spatial sound samples
...
-- @param string sampleName Sample element name
-- @param string baseDir Sample file path base directory
-- @param int loops Loop count of sample, defaults to 1. A value of 0 will loop indefinitely.
-- @param string audioGroup Sample audio group
function SoundManager:loadSample2DFromXML(xmlFile, baseKey, sampleName, baseDir, loops, audioGroup)
    ...
    if isValid then
        ...
        loadSample(sample.soundSample, sample.filename, false) -- false -> 2D sound
...
function SoundManager:loadSampleFromXML(xmlFile, baseKey, sampleName, baseDir, components, loops, audioGroup, i3dMappings, modifierTargetObject)
    ...
    local isValid, _, definitionXmlFile, sampleKey, linkNode = self:validateSampleDefinition(
        xmlFile, baseKey, sampleName, baseDir, audioGroup, false, components, i3dMappings, externalSoundsFile)
...
function SoundManager:loadSampleAttributesFromXML(sample, xmlFile, key, baseDir, defaultLoops, requiresFile)
    ...
    sample.innerRadius = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key.."#innerRadius"), sample.innerRadius), 5.0)
    sample.outerRadius = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key.."#outerRadius"), sample.outerRadius), 80.0)

    sample.volumeScale = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key.."#volumeScale"), sample.volumeScale), 1.0)
    sample.pitchScale = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key.."#pitchScale"), sample.pitchScale), 1.0)
    sample.lowpassGainScale = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key.."#lowpassGainScale"), sample.lowpassGainScale), 1.0)

    sample.loopSynthesisRPMRatio = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key.."#loopSynthesisRPMRatio"), sample.loopSynthesisRPMRatio), 1.0)
    ...
    sample.indoorAttributes.pitch = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key..".pitch#indoor"), sample.indoorAttributes.pitch), 1.0)
    ...
    sample.indoorAttributes.lowpassCutoffFrequency = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key..".lowpassCutoffFrequency#indoor"), sample.indoorAttributes.lowpassCutoffFrequency), 0.0) -- by default this is defined by the group (default 5000hz, resonance 2)
    sample.indoorAttributes.lowpassResonance = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key..".lowpassResonance#indoor"), sample.indoorAttributes.lowpassResonance), 0.0)

    sample.outdoorAttributes = Utils.getNoNil(sample.outdoorAttributes, {})
    sample.outdoorAttributes.volume = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key..".volume#outdoor"), sample.outdoorAttributes.volume), 1.0)
    sample.outdoorAttributes.pitch = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key..".pitch#outdoor"), sample.outdoorAttributes.pitch), 1.0)
    sample.outdoorAttributes.lowpassGain = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key..".lowpassGain#outdoor"), sample.outdoorAttributes.lowpassGain), 1.0)
    sample.outdoorAttributes.lowpassCutoffFrequency = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key..".lowpassCutoffFrequency#outdoor"), sample.outdoorAttributes.lowpassCutoffFrequency), 0.0)
    sample.outdoorAttributes.lowpassResonance = Utils.getNoNil(Utils.getNoNil(getXMLFloat(xmlFile, key..".lowpassResonance#outdoor"), sample.outdoorAttributes.lowpassResonance), 0.0)
...
--#debug                        if sample.modifierTargetObject == nil or sample.modifierTargetObject.propertyState == nil or sample.modifierTargetObject.propertyState ~= Vehicle.PROPERTY_STATE_SHOP_CONFIG then
...
---Stops the sample
-- @param table sample sample object
-- @param float delay set delay in ms for stopping the sound, set automatically for loop systehsis samples, 0 otherwise
...
-- @param boolean true if sound should be played as inside building, else false
...
-- @return boolean isIndoor true if inside building mode is active else false
...
function SoundManager.registerSampleXMLPaths(schema, basePath, name)
    ...
    schema:register(XMLValueType.FLOAT, soundPath .. ".pitch#indoor", "Indoor pitch", 1.0)
    schema:register(XMLValueType.FLOAT, soundPath .. ".lowpassGain#indoor", "Indoor lowpass gain", 0.8)

    schema:register(XMLValueType.FLOAT, soundPath .. ".lowpassCutoffFrequency#indoor",      "Indoor lowpass cutoff frequency", 5000.0)
    schema:register(XMLValueType.FLOAT, soundPath .. ".lowpassResonance#indoor",            "Indoor lowpass resonance", 2.0)
    schema:register(XMLValueType.FLOAT, soundPath .. ".lowpassCutoffFrequency#outdoor",     "Outdoor lowpass cutoff frequency", 5000.0)
    schema:register(XMLValueType.FLOAT, soundPath .. ".lowpassResonance#outdoor",           "Outdoor lowpass resonance", 2.0)

    schema:register(XMLValueType.FLOAT, soundPath .. ".volume#outdoor", "Outdoor volume", 1.0)
    schema:register(XMLValueType.FLOAT, soundPath .. ".pitch#outdoor", "Outdoor pitch", 1.0)
    schema:register(XMLValueType.FLOAT, soundPath .. ".lowpassGain#outdoor", "Outdoor lowpass gain", 1.0)

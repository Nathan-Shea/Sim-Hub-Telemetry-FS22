...
function EnvironmentalScore.new(pfModule, customMt)
    ...
    self.ui.iconOverlay:setUVs(GuiUtils.getUVs({512, 64, 64, 64}))
    ...
    self.ui.gradientOverlay:setUVs(GuiUtils.getUVs({194, 27, 252, 12}))
    ...
    self.ui.colorMainUI = {0.0227, 0.5346, 0.8519, 1.0}
...
function EnvironmentalScore:onDraw(element, ingameMap)
    if g_currentMission:getFarmId() ~= FarmManager.SPECTATOR_FARM_ID then
        ...
        for farmlandId, _farmId in pairs(g_farmlandManager.farmlandMapping) do
            if _farmId == farmId then
                if g_fieldManager.farmlandIdFieldMapping[farmlandId] == nil then
                    local farmland = g_farmlandManager:getFarmlandById(farmlandId)
...
function EnvironmentalScore:onDrawFieldNumber(element, ingameMap, x, y, field)
    if field.farmland ~= nil then
        ...
        if field.farmland.totalFieldArea ~= nil and field.farmland.totalFieldArea > 0.01 then
...
function EnvironmentalScore:drawFarmlandScore(x, y, dataKey, farmlandId, zoom)
    ...
    if self.currentInputHelpMode == GS_INPUT_HELP_MODE_GAMEPAD then
        if 0.5 > windowX and 0.5 < windowX + windowWidth then
            if 0.5 > windowY and 0.5 < windowY + windowHeight then
                isHovering = true
            end
        end
    elseif g_inputBinding.mousePosXLast ~= nil then
        if g_inputBinding.mousePosXLast > windowX and g_inputBinding.mousePosXLast < windowX + windowWidth then
            if g_inputBinding.mousePosYLast > windowY and g_inputBinding.mousePosYLast < windowY + windowHeight then
                isHovering = true
            end
        end
    end

...
function WeedSpotSpray.initSpecialization()
    g_configurationManager:addConfigurationType("weedSpotSpray", g_i18n:getText("configuration_weedSpotSpray"), "weedSpotSpray", nil, nil, nil, ConfigurationUtil.SELECTOR_MULTIOPTION)
                ...
                if foldingConfigIndex == self.configurations["folding"] or (self.configurations["folding"] == nil and foldingConfigIndex == 1) then
                    ...
                    if g_precisionFarming ~= nil then
                        ...
                        if effectNode ~= nil then
                            ...
                            entry.effect.fadeCur = {-1, 1}
                            entry.effect.fadeDir = WeedSpotSpray.EFFECT_DIRECTION_OFF
                            entry.effect.state = ShaderPlaneEffect.STATE_OFF

                            setShaderParameter(entry.effect.node, "fadeProgress", entry.effect.fadeCur[1], entry.effect.fadeCur[2], 0.0, 0.0, false)
                            setShaderParameter(entry.effect.node, "offsetUV", math.random(), math.random(), 0.0, 0.0, false)
                        end
                    end
            ...
            if effect.node ~= nil then
                if nozzleNode.lastIsActive ~= nozzleNode.isActive then
                    if nozzleNode.isActive then
                        ...
                        if effect.state ~= ShaderPlaneEffect.STATE_OFF and effect.state ~= ShaderPlaneEffect.STATE_TURNING_OFF then
                ...
                if effect.state == ShaderPlaneEffect.STATE_TURNING_OFF or effect.state == ShaderPlaneEffect.STATE_TURNING_ON then
                    ...
                    setShaderParameter(effect.node, "fadeProgress", effect.fadeCur[1], effect.fadeCur[2], 0.0, 0.0, false)
                    ...
                    elseif effect.state == ShaderPlaneEffect.STATE_TURNING_ON then
                        if effect.fadeCur[1] == 1 and effect.fadeCur[2] == 1 then
